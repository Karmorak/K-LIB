...
//		[ ] MasterRenderer ist momentan in Running müsste aber pro Window sein da jedes Window einen eigenen Renderer braucht solange ich nur ein Window nutze ist das aber nicht so schlimm.
...
//  	[ ] GlobalInput mit in Input integrieren und erneuern
	...
	public static void START(Running run, WindowOptions options, int req_lib_version) {
		START(run, options, req_lib_version, null);
	}
	...
	public static void OPEN_WINDOW(boolean asnewthread, Running run, WindowOptions options) {
		if(!asnewthread) {		
			...
			run.start(new Window(options), null);
		}
	}
	...
	private static void INIT(int version, String app_name, String app_version) {
		if(!isINIT) {
			...
			System.out.println("OS: " + System.getProperty("os.name") + " v." + System.getProperty("os.version") + " " + System.getProperty("os.arch") + " | " + "HeapSize:" + (Runtime.getRuntime().maxMemory() / 1024 / 1024) + "MB");
			System.out.println("Location: " + System.getProperty("user.dir") + " | " + "Save-Dir: " + System.getProperty("user.home") + "| " + "Language: " + System.getProperty("user.language"));
...
//			KLIB.class.getProtectionDomain().getCodeSource().getLocation();
			System.out.println("Save-Location: " + io.getLocation(KLIB.class)); //when in eclipse file:/E:/Eclipse/workspace/lwjgl-lib/bin/ when compiled file:/C:/Users/marce/Desktop/test.jar
...
//				loadingSpinner.render();
	...
	public static class system {
		...
		public static String DateAndTime(boolean seconds) {
			if (seconds)
				return Date() + " " + Time();
			else
				return Date() + " " + Time("HH:mm");
	...
	public static abstract class graphic {
		...
		public static int getWidth() {
			return (int) Window.windows.getFirst().getBounds().getWidth();
		}
		...
		public static int getHeight() {
			return (int) Window.windows.getFirst().getBounds().getHeight();
		}
	...
	public static abstract class input {
		...
		public static void HorizontalShiftScroll(boolean value) {
			Input.option_scroll_shift = value;
		}

		public static void updateButtons(boolean value) {
			Input.updateButtons(value);
		}
